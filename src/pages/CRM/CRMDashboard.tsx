...
const CRMDashboard = () => {
    ...
    }, [user, dateRange, crmType]);

    const fetchAnalytics = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(getApiUrl(`/api/crm/analytics?dateRange=${dateRange}&crmType=${crmType}`));
    ...
    // Transform daily trends for charts
            ...
            <div className="lg:pl-64 p-6">
                ...
                <div className="bg-white rounded-lg shadow p-4 mb-6">
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                        Showing data for {dateRange === 'today' ? 'today' : `last ${dateRange} days`} {crmType !== "all" ? `(${crmType.toUpperCase()} CRM)` : "(All CRMs)"}                    </p>
                    <Button
                ...
                {/* Dashboard Content with Ref for PDF */}
                <div ref={dashboardRef}>
                    {analytics && (
                        <>
                            ...
                            {/* CRM Type Statistics */}
                            <div id="crm-type-stats" className="bg-white rounded-lg shadow p-6 mb-6">
